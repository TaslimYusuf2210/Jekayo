...
export class OrderService implements OnChanges {
  ...
  addOrder(product: Orders) {
    ...
    const existingProduct = cartOrders.find(
      (item: Orders) => item.id == product.id
    );
...
calculateOrderAmount() {
  let cartOrders = this.globalService.getData('orders');
  let total  = cartOrders.reduce((total:number, item:Orders) => total + (item.price * item.quantity),0)
  return total
}
}
